...
* Changes
  I apologies to anyone who had already started using this package for the name change.
* Basic Usage
 ...
 If you already use ~use-package~. Then, the config below works, although this package is not yet part of melpa so, either have it manually be in the ~load-path~, or use straight.el.
  #+begin_src emacs-lisp
    (use-package modules
      ...
      (modular-config-command-line-args-process))
  #+end_src
Modules can also be loaded interactively by using the ~modular-config-load~ function.
...
#+begin_src emacs-lisp
(modular-config-load '(space separated list of modules))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (when (modular-config-loaded-p 'module-name)
    ;; Dependent Code
    )
#+END_SRC
* Example Module
  ...
  #+begin_src emacs-lisp
    (modular-config-load '(versioncontrol syntaxcheck completion))
